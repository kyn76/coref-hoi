...
def check_mentions_in_k_best_ant_gold_csv():
    for i in range(NB_DOCUMENTS):
        ...
        kbest_spans = []
        kbest_file = open(f"evaluation/{i}-k_best_ant_gold_bound.csv", "r")
...
def count_recorded_correct_antecedents():
    ...
    for i in range(NB_DOCUMENTS):
        ...
        gold_reader = list(csv.DictReader(gold_file))
        for gold_row in gold_reader:
            gold_antecedent_dict[(int(gold_row["anaphor_start"]), int(gold_row["anaphor_end"]))] = (int(gold_row["antecedent_start"]), int(gold_row["antecedent_end"]))
        gold_file.close()
...
if __name__ == '__main__':
    ...
    count_recorded_correct_antecedents()