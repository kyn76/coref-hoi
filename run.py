...
class Runner:
    ...
    def evaluate_from_csv(self, model, tensor_examples, stored_info, step, official=False, conll_path=None, tb_writer=None, gold_boundaries=True, k=1):
        ...
        for rank in range(1, k + 1):
            for i, (doc_key, tensor_example) in enumerate(tensor_examples):
                ...
                with open(f"kba-antecedents-csv/{i}-gold_antecedents.csv", "r") as gold_file:
                    gold_reader = list(csv.DictReader(gold_file))
                with open(f"kba-antecedents-csv/{i}-{pred_filename}.csv", "r") as pred_file:
    ...
    def bell_tree_process(self, model, tensor_examples, stored_info, step, official=False, conll_path=None, tb_writer=None, intra_aggregations=["max"], nbs_beams=[1], verbose=False):
        logger.info('Step %d: evaluating on %d samples...' % (step, len(tensor_examples)))
        model.to(self.device)

        metric_file = open("kba-metrics/bt-metrics-gold_boundaries.csv", "w")
        metric_file.write("intra_cluster_aggregation,nb_beams,eval_avg_p,eval_avg_r,eval_avg_f,conll_md_p,conll_md_r,conll_md_f,conll_muc_p,conll_muc_r,conll_muc_f,conll_bcub_p,conll_bcub_r,conll_bcub_f,conll_ceafe_p,conll_ceafe_r,conll_ceafe_f,conll_avg_f\n")
        
        for intra_aggregation in intra_aggregations:
            for nb_beams in nbs_beams:
                evaluator = CorefEvaluator()
                doc_to_prediction = {}
                model.eval()
                for i, (doc_key, _) in enumerate(tensor_examples):
                    ...
                    predicted_clusters = model.update_evaluator_v2(span_starts, span_ends, predicted_antecedent_idx, gold_clusters, evaluator)
                    doc_to_prediction[doc_key] = predicted_clusters
                    logger.info(f"Bell tree running... [{intra_aggregation} agg. | {nb_beams} beams] (doc {i+1}/{len(tensor_examples)})")
                ...
                metric_file.write(f"{intra_aggregation},{nb_beams},{p*100:.2f},{r*100:.2f},{f*100:.2f},{conll_results['muc']['md_p']},{conll_results['muc']['md_r']},{conll_results['muc']['md_f']},{conll_results['muc']['p']},{conll_results['muc']['r']},{conll_results['muc']['f']},{conll_results['bcub']['p']},{conll_results['bcub']['r']},{conll_results['bcub']['f']},{conll_results['ceafe']['p']},{conll_results['ceafe']['r']},{conll_results['ceafe']['f']},{official_f1:.4f}\n")
        metric_file.close()
